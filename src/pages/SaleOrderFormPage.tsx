...
import type { SaleOrderRequest, SaleOrderStatus } from "../types/saleOrder";
...
/**
 * All possible sale order statuses
 */
const statuses: SaleOrderStatus[] = ["draft", "ordered", "partially_sent", "send", "cancelled"];

/**
 * Return sale order form page
 * @returns Sale order form page
 */
function SaleOrderFormPage() {
    const [formData, setFormData] = useState<SaleOrderRequest>({
        orderNumber: "",
        customerId: "",
        status: "draft",
        orderDate: "",
    ...
    const [stockShelves, setShelves] = useState<StockShelf[]>([]);
    ...
    /**
     * Load sale order data if editing
     */
    useEffect(() => {
        if (saleOrderId) {
            api.get(`/sale-order/${saleOrderId}`)
                .then(response => setFormData(response.data))
                .catch(error => setError(error.response?.data?.error || "Sale order retrieval failed"));
        }
    }, [saleOrderId]);

    /**
     * Load customers, products, stocks and shelves for dropdowns
     */
    useEffect(() => {
    ...
    /**
     * Add new empty item row to the order
     */
    const handleAddItem = () => {
        const newItem = {
            productName: "",
    ...
    /**
     * Remove a item row by its index
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        try {
            if (saleOrderId) {
                await api.put(`/sale-order/${saleOrderId}`, formData);
            } else {
                await api.post("/sale-order", formData);
            }

            navigate("/sale-orders");
        } catch (error) {
            if (error instanceof AxiosError) {
                setError(error.response?.data?.error || "Sale order " + (saleOrderId ? "update" : "creation") + " failed")
            } else {
    ...
    return (
        <div className="p-4">
            <h1 className="fs-4 fw-bold">Sale order form</h1>
            <form className="bg-white rounded shadow p-4" onSubmit={handleSubmit}>
                ...
                <label className="form-label mt-3 fw-medium">Customer
                    <select
                    ...
                    >
                        ...
                        {customers.map(customer => (
                            <option key={customer._id} value={customer._id}>{customer.first_name} {customer.last_name}</option>
                        ))}
                    </select>
                </label>
                <label className="form-label mt-3 fw-medium">Status
                    <select
                        ...
                        onChange={(event) => {
                            setFormData({ ...formData, status: event.target.value as SaleOrderStatus });
                            setError("");
                        }}
                    >
                        {statuses.map(status => (
